...
class ctThermoTransport:
    def __init__(
        ...
        """
        A class providing functions to access cantera mechanism and thermo/transport
        data, required in polynomial fitting.
    ...
    def get_thermo_fit_type(self, species_name):
        ...
        return:  [NasaPoly2 (NASA7), Nasa9PolyMultiTempRegion (NASA9), Shomate]
        """
    ...
    def evaluate_properties(self):
        ...
        for sp_i in gas.species_names:
            ...
            # standard properties
            self.cp0_over_R[i] = (
                gas.species(i).thermo.cp(self.T_std) / ct.gas_constant
            )  # this calls molar cp <==> consistent
    ...
    def evaluate_mixture_properties(self, mixture_name, X):
        ...
        - overwrites class level array-type declarations with
          smalled dimension data types.
        """
        # Initialise a free flame object
        gas = ct.Solution(self.mechanismFile)
        p0 = ct.one_atm  # - not utilised
        ...
        if self.verbose:
            print(
                "Evaluating thermophysical properties for mixture "
                + mixture_name
                + ": "
                + str(X)
            )
    ...
    def check_temperature_limits(self, species_name):
        """
        Print a warning if user given temperature limits are
        out of the original data limits.
        """
        minT_ref = self.gas.species(
            self.gas.species_index(species_name)
        ).thermo.min_temp
        minT_user = np.min(self.T)
        maxT_ref = self.gas.species(
            self.gas.species_index(species_name)
        ).thermo.max_temp
