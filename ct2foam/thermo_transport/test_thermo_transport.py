...
class TestThermoTransport(unittest.TestCase):
    ...
    def test_transport_fit(self):
        ...
        success = ct2foam_utils.transport_fit_quality(
            data, transport_fits, test_data_dir, False, 0.02, 0.08, 5e-3
        )
    ...
    def test_transport_sanity(self):
        ...
        As, Ts, _, poly_mu, poly_kappa, log_poly_mu, log_poly_kappa = (
            ct2foam_utils.fit_ct_transport(data)
        )

        mu_s = tr_fitter.sutherland(T, As[i], Ts[i])
        kappa_s = tr_fitter.euken(mu_s, cv_mole, W, R)
        mu_logp, kappa_logp = tr_fitter.eval_log_polynomial(
            log_poly_mu[i, :], log_poly_kappa[i, :], T
        )
    ...
    def test_nasa9(self):
        thermo = ct_properties.ctThermoTransport(
            Path(test_data_dir, "h2o2_mod.yaml"), verbose=False
        )
    ...
    def test_mixture_transport(self):
        ...
        mu_p, kappa_p = tr_fitter.eval_polynomial(
            poly_mu[0, :], poly_kappa[0, :], data.T
        )
    ...
    def test_thermo_foam_writer(self):
        ...
        # The following more elaborate test fails if ran on different
        # architechtures (floating point differences)
        # Writes a dictionary which is used by
        # test_data/OF_reference/Test_thermoMixture.C
        """
        ...
        ct2foam_utils.ct2foam_thermo_writer(
            sp_file,
            foam_file,
            r_file,
            data,
