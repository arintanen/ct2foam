...
def warning_sutherland(
    output_dir: Path, specie_name: str, err_mu: float, std: float, err_kappa: float
):
    warning = (
        "\n\tWarning : "
        + str(specie_name)
...
def warning_polynomial(
    output_dir: Path, specie_name: str, err_mu: float, err_kappa: float, log_type: bool
):
    if log_type:
...
def warning_nasa7_err(
    ...
    warning = (
        "\n\ttWarning : "
        + str(name)
        + " NASA7 fit error is larger than tolerance "
        + str(rel_tol)
        + "\tcp error: "
        + str(err_cp)
        + "\n"
...
def warning_nasa7_continuity(name: str, output_dir: Path):
    warning = (
        "\n\tWarning : "
        + str(name)
        + ": the new NASA polynomial fit is not continuous."
    )
    print(warning)
    with open(Path(output_dir, _log_file), "a") as f:
        print(warning, file=f)
