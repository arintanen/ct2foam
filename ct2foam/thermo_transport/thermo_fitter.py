...
from . import (
    lsqlin,
)  # 3rd party matlab like lsqlin, providing better performance than scipy (in 2016)

"""
See the following links for further information on the NASA polynomial fitting:
...
This function fits polynomials to specific heat, using NASA 7 term format from
SP-272. Gordon and McBride 1971.
...
- Fit the specific heat only and use the analytical integration to obtain the
  additional coefficients for entropy and enthalpy.
- The specific heat fit constrains the derivative to be continuous at
  Tcommon and the value of specific heat at 298.15 K is constrained to
  be the specified value of the standard state (enthalpy of formation).
- The entropy and enthalpy values at the midpoints are forced to be continuous
  by analytical constraining.
- This procedure produces high quality fits as long as the input entropy and
  enthalpy are thermodynamically consistent with the specific heat data.
  This will always be the case if the thermodynamic data have been derived
  from partition functions and presumably when refitting existing fit functions.
- Be cautious in refitting!

2) Original data from low-quality fit or from experiments
- Create a simultaneous least squares fit considering the whole system with
  prescribed constraints.


Fitting procedure:

Solve linear constrained l2-regularized least squares by an algorithm equivalent
to Matlab's lsqlin, which is actually a wrapper around the CVXOPT QP solver.
...
def consistency(T, Tmid, c_lo, c_hi, cp_over_R, h_over_RT, s_over_R, abs_tol=1e-6):
    """
    Check if NASA7 coefficients provide consistent evaluation against the reference
    data in terms of L2 error.
    cp_over_R, h_over_RT and s_over_R: [M,] sized array.
...
def continuity(Tmid, c_lo, c_hi, cp_tol=1e-6, cpdT_tol=0.01, h_tol=1e-6, s_tol=1e-6):
    ...
    - C1 continuity (not mandatory but recommended):
      For some mechanism fits C1 continuity is not guaranteed
      --> a loose tolerance is chosen
    :coeffs:
...
def correct_coeffs(
    coeffs: np.ndarray, Tcommon: float, dhf_over_R: float, s0_over_R: float
):
    """
    ...
    coeffs[5] = dhf_over_R - (
        coeffs[0] * __T_std
        + coeffs[1] * ((1.0 / 2.0) ** (1.0 / 2.0) * __T_std) ** 2
        + coeffs[2] * ((1.0 / 3.0) ** (1.0 / 3.0) * __T_std) ** 3
        + coeffs[3] * ((1.0 / 4.0) ** (1.0 / 4.0) * __T_std) ** 4
        + coeffs[4] * ((1.0 / 5.0) ** (1.0 / 5.0) * __T_std) ** 5
    )
    ...
    coeffs[6] = s0_over_R - (
        coeffs[0] * np.log(__T_std)
        + coeffs[1] * __T_std
        + coeffs[2] * ((1.0 / 2.0) ** (1.0 / 2.0) * __T_std) ** 2
        + coeffs[3] * ((1.0 / 3.0) ** (1.0 / 3.0) * __T_std) ** 3
        + coeffs[4] * ((1.0 / 4.0) ** (1.0 / 4.0) * __T_std) ** 4
    )
    # Define coeffs[6+7] by ensuring continuity at Tcommon
    def s_sum_term_l(T):
        return (coeffs[0] * np.log(T)
            + coeffs[1] * T
            + coeffs[2] * ((1.0 / 2.0) ** (1.0 / 2.0) * T) ** 2
            + coeffs[3] * ((1.0 / 3.0) ** (1.0 / 3.0) * T) ** 3
            + coeffs[4] * ((1.0 / 4.0) ** (1.0 / 4.0) * T) ** 4)
    def s_sum_term_h(T):
        return (coeffs[0 + 7] * np.log(T)
            + coeffs[1 + 7] * T
...
def fit_nasapolys_cp(
    T0: np.ndarray,
    Tc_i: int | np.signedinteger,
    ...
    Fit the specific heat only and use the analytical integration to obtain the
    additional coefficients for entropy and enthalpy. The specific heat fit constrains
    the derivative to be continuous at Tcommon and the value of specific heat at
    298.15 K is constrained to be the specified value of the standard
    state (enthalpy of formation). The entropy and enthalpy values at the midpoints
    are forced to be continuous by analytical consrtaining.

    T0: temperature sampling. Supports 1d-arrays only [M,] sized.
    Tc_i is the common temperature index dividing the temperature data into low/high
    regimes in the T0 array
    ...
    # solving additional constant by means of conservation of enthalpy and entropy
    # and ensuring C0 continuity at T=Tcommon
...
def fit_nasapolys_full(
    ...
    # cp_over_R : C0 continuity at Tcommon
    ...
    - if forcing the constraints to the linear solution (below),
    result are typically less good
