...
def fit_ct_transport(data, poly_order=3):
    ...
    for i in range(N):
        ...
        poly_mu[i], poly_kappa[i] = tr_fitter.fit_polynomial(
            data.T, data.mu[i, :], data.kappa[i, :], poly_order
        )
        logpoly_mu[i], logpoly_kappa[i] = tr_fitter.fit_log_polynomial(
            data.T, data.mu[i, :], data.kappa[i, :], poly_order
        )
...
def transport_fit_quality(
    data,
    transport_fits,
    output_dir,
    plot=True,
    rel_tol_sutherland=2e-2,
    rel_tol_Euken=2e-2,
    rel_tol_poly=5e-3,
):
    """
    ...
    for i in range(N):
        ...
        err_mu_sutherland, err_kappa_sutherland = tr_fitter.error_sutherland(
            mu[i, :], kappa[i, :], T, As[i], Ts[i], cv_mole[i], W[i], ct.gas_constant
        )
        err_mu_poly, err_kappa_poly = tr_fitter.error_polynomial(
            mu[i, :], kappa[i, :], poly_mu[i, :], poly_kappa[i, :], T
        )
        err_mu_logpoly, err_kappa_logpoly = tr_fitter.error_log_polynomial(
            mu[i, :], kappa[i, :], logpoly_mu[i, :], logpoly_kappa[i, :], T
        )

        sutherland_ok = (err_mu_sutherland < rel_tol_sutherland) and (
            err_kappa_sutherland < rel_tol_Euken
        )
        polynomial_ok = (err_mu_poly < rel_tol_poly) and (err_kappa_poly < rel_tol_poly)
        logpolynomial_ok = (err_mu_logpoly < rel_tol_poly) and (
            err_kappa_logpoly < rel_tol_poly
        )

        if not sutherland_ok:
            success = False
            warning_msg.warning_sutherland(
                output_dir, names[i], err_mu_sutherland, std[i], err_kappa_sutherland
            )

        if not polynomial_ok:
            success = False
            warning_msg.warning_polynomial(
                output_dir, names[i], err_mu_sutherland, err_kappa_sutherland, False
            )

        if not logpolynomial_ok:
            success = False
            warning_msg.warning_polynomial(
                output_dir, names[i], err_mu_sutherland, err_kappa_sutherland, True
            )
...
def plot_transport_comparison(
    ...
    mu_poly, kappa_poly = tr_fitter.eval_polynomial(
        poly_coeffs_mu, poly_coeffs_kappa, T
    )
...
def ct_thermo_consistency(thermo, sp_i, output_dir):
    ...
    cp_over_R, h_over_RT, s_over_R = nasa_normalisation(
        thermo.T, thermo.cp, thermo.h, thermo.s
    )
    i = thermo.gas.species_index(sp_i)
    consistent_data = th_fitter.consistency(
        thermo.T,
        nasa7_coeffs[0],
        nasa7_coeffs[8:15],
        nasa7_coeffs[1:8],
...
def ct_thermo_continuity(thermo, sp_i):
    ...
    """
    nasa7_coeffs = thermo.get_nasa7_coeffs(sp_i)
    continuous_fit = th_fitter.continuity(
        nasa7_coeffs[0], nasa7_coeffs[8:15], nasa7_coeffs[1:8]
    )
...
def plot_nasa7_comparison(
    name, T, cp_over_R, h_over_RT, s_over_R, Tmid, coeffs_lo, coeffs_hi, output_dir
):
    ...
    for sp_i in thermo.gas.species_names:
        ...
        if thermo.is_nasa7(sp_i):
            ...
            if continuos and equal_Tmid:
                ...
                coeffs_lo, coeffs_hi = th_fitter.fit_nasapolys_cp(
                    thermo.T,
                    Tci,
                    cp_over_R[i, :],
                    thermo.cp0_over_R[i],
                ...
                coeffs_lo, coeffs_hi = th_fitter.fit_nasapolys_full(
                    thermo.T,
                    Tci,
                    cp_over_R[i, :],
                    h_over_RT[i, :],
...
def nasa7_fit_quality(data, thermo_fits, output_dir, plot=True):
    ...
    success = True
    for i in range(N):
        err_cp, err_h, err_s = th_fitter.error_nasa7(
            data.T,
            data.cp[i, :],
            data.h[i, :],
            data.s[i, :],
        ...
        if not fit_ok:
            success = False
            warning_msg.warning_nasa7_err(
                data.names[i], rel_tol, err_cp, err_h, err_s, output_dir
            )

        continuous_fit = th_fitter.continuity(
            data.Tmid, nasa_coeffs_lo[i, :], nasa_coeffs_hi[i, :]
        )
        if not continuous_fit:
            success = False
        ...
        if (not (fit_ok and continuous_fit)) and plot:
            ...
            fig_dir.mkdir(exist_ok=True)
            cp_over_R, h_over_RT, s_over_R = nasa_normalisation(
                data.T, data.cp, data.h, data.s
            )
            plot_nasa7_comparison(
                data.names[i],
                data.T,
                cp_over_R[i, :],
                h_over_RT[i, :],
